...
var dbURL = "mongodb://" + settings.mongodb.user + ":" + settings.mongodb.pwd + "@" + settings.mongodb.host + ":" + settings.mongodb.port + "/" + settings.mongodb.name;
...
var DB = function() {
   this.mongoClient = {};
   // var filename = path.join(path.dirname(__dirname).replace('app', ''), 'server/config/mongotable.json');
   var filename = path.join(path.dirname(__dirname), 'config/mongotable.json');
...
/**
   聚合查询
*/
DB.prototype.aggregate = function(table_name, conditions, callback) {
   var node_model = this.getConnection(table_name);
   node_model.aggregate(conditions, callback);
}
